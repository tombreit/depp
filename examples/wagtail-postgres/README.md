...
# 3. Point depp.toml at a host you control (fqdn, contact_email), then:
depp provision depp.toml        # one-time host setup (podman, user, Apache)
depp deploy depp.toml           # build, transfer, run

# 4. First-run database setup (run once after the first deploy).
depp exec depp.toml -- python manage.py migrate
...
- Rename `wagtail-demo` everywhere (`depp.toml` `name`, the pod/volume/claim
